...
// NewKeyProxy creates a reverse proxy handler that reads the upstream base URL
// from the validation result in context and forwards the original request path
// intact (no prefix stripping — the key determines the backend).
func NewKeyProxy() http.Handler {
	return http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		proxy.Director = func(req *http.Request) {
			...
			// Forward the original path intact — no prefix stripping.
			// The backend receives the same path as the gateway request.
